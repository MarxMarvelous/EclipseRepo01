			...
			//_testCase = "signupValid";
  			switch (_testCase) {
				...
				case "signinValidRemember": //without remember me
					...
					if(_emailAddress.equals(_email)) {
						System.out.println("Correct signin email: "+_email);
					} else {
						fail("Fail - bad email address: "+_emailAddress);
					}
					...
					_emailAddress = catalog.getEmailAddress();
					if(_emailAddress.equals(_email)) {
						System.out.println("PASS Correct signin remembered: "+_email);
					} else {
						fail("Fail - bad email address: "+_emailAddress);
					}
				...
				case "signinBlankEmail": 	// fails, issue called out 3/5/14
					...
					if(_errorMsg.equals("Email address is required.")) {
				...
				case "signinEmailBlankPW": 	// fails, issue called out 3/5/14
					...
					if(_errorMsg.equals("Password is required")) {
						System.out.println("PASS Correct error advisory: "+_errorMsg);
					} else {
				...
				case "signinBadUsername": 
					...
					if(_errorMsg.equals("The email address or password is incorrect.")) {
						...
						fail("FAIL - requires bad username advisory not: "+_errorMsg);
					}
				...
				case "signinBadPW": 
					signup.helloPlatform(_td.getBaseUrl());
					_email = _username; //bad email
					signup.signInTest(_email, _password+"xxx", 0);  //bad password
					...
					if(_errorMsg.equals("The email address or password is incorrect.")) {
						...
						fail("FAIL - requires bad password advisory not: "+_errorMsg);
					}
