...
import shoppostPages.SemanticPage;
...
public class ShoppostCatalog {
	...
	@Test
	  public void test_productCatalog() throws Exception {
		...
		semantic = new SemanticPage(driver);
			...
			_folderTestCase = _td.getUIData().getTcFolder()+_testCase;
  			switch (_testCase) {
				...
				case "addProductValidURL": // new product displayed in share modal
					String _semProductTitle, _semProductPrice;
					int _preCount, _postCount;
					signup.helloPlatform(_td.getCatalogTests().getProductURL()); //semantic product page (walmart OR BestBuy page)| enable json for the URL and store abbreviation
					_semProductTitle = semantic.getProductTitle(_td.getCatalogTests().getStore());
					System.out.println(_semProductTitle);
					_semProductPrice = semantic.getProductPrice(_td.getCatalogTests().getStore());
					System.out.println(_semProductPrice);
					
					signup.helloPlatform(_td.getBaseUrl());
					signup.signInTest(_username, _password, 0);
					analyticsReporter.toCatalog();
					_preCount = catalog.getProductCount();
					System.out.println("Pre-count: "+_preCount);
					catalog.addProduct();
					Thread.sleep(5000);
					...
					catalog.hoverLatestsProduct();  //hovers latest product
					Thread.sleep(3000);
					catalog.getShare();  //opens share overlay of hovered product
					shareModal.goShare(); 
					
					Thread.sleep(10000);
					
					if(_semProductTitle.equals(shareModal.getProductName())) {
						System.out.println("PASS Correct Product Name: "+_semProductTitle);
					} else {
						fail("FAIL - Semantic product title and shoppost do not match: "+_semProductTitle+" :: "+shareModal.getProductName());
					}
					shareModal.closeShareModal();
					catalog.closeAdd();  //close add shoppost
					_postCount = catalog.getProductCount();
					if(_preCount+1 == _postCount) {
						System.out.println("PASS! product count increased by 1.");
					}else{
						System.out.println("ERROR - product count is off.");
					}
				...
				case "addProductAndCancel": // see error message
				...
				case "shareModalTest": //Opens share modal window for random product
					signup.helloPlatform(_td.getBaseUrl()); //go to shoppost login
					analyticsReporter.toCatalog();
				...
				case "addProductViaDashboard": 	//
					...
					signup.helloPlatform(_td.getBaseUrl());
					signup.signInTest(_username, _password, 0);
					analyticsReporter.toCatalog();  //after signin go to shopppost list
					preCt = catalog.getProductCount();
					catalog.toDash();
					catalog.addProduct();
					Thread.sleep(1000);
				...
				case "noProductDisplay": 
					signup.helloPlatform(_td.getBaseUrl());
					r = rand.nextInt(1000);
					signup.signUpTest(_td.getSignupinTests().getNewUsername()+r+"@mark.com", _password, 0);
					//analyticsReporter.toCatalog();
					Thread.sleep(1000);
					
					_errorMsg = catalog.getMessage();
					if(_errorMsg.equals("Must be a valid email address.")) {
						System.out.println("PASS Correct no product message: "+_errorMsg);
					} else {
						fail("FAIL - requires 'no products' message not: "+_errorMsg);
					}
				...
				case "viewRandomStats": //View random product Stats from product hover
					...
					if(shareModal.isDonutVisible()) {
						System.out.println("PASS - Individual shoppost stats visible.");
					}else{
						fail("FAIL - individual stats failed to be displayed.");
					}
